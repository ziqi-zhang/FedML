...
def load_server(args, dataset ,device, model_trainer):
    if args.aggr == "fedavg":
        server = FedAvgAPI(dataset, device, args, model_trainer)
    elif args.aggr == "predavg":
        server = PredAvgAPI(dataset, device, args, model_trainer)
    else:
...
if __name__ == "__main__":
    ...
    logger.info(device)
    args.exp_name = (
        f"FedAVG [{args.run_tag}] {args.dataset}-{args.model}-" + 
        f"{args.partition_method}{str(args.partition_alpha)}-r{str(args.comm_round)}" +
    ...
    server = load_server(args, dataset, device, model_trainer)
    server.train()
    # server.local_test_on_global_dataset(args.comm_round)
    # server.local_test_on_other_client_dataset(args.comm_round)
